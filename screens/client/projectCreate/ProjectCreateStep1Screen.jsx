  ...
  return (
    <View className="flex-1 bg-white" contentContainerStyle={{ paddingBottom: 100 }}>
      ...
      <ScrollView className="px-4 pt-6">
        ...
        <Text className="text-x text-black mb-1">Название проекта</Text>
        <TextInput
          value={title}
        ...
        <Text className="text-x text-black mb-1">Описание проекта</Text>
        <TextInput
          value={description}
        ...
        <Text className="text-x text-black mb-2">Фотографии объекта</Text>
        <ScrollView horizontal className="mb-4">
          {sourceImages.map((img, i) => (
            <Image key={i} source={{ uri: img.uri }} className="w-24 h-24 mr-2 rounded-md" />
          ))}
          <TouchableOpacity onPress={() => pickImage(setSourceImages)} className="w-24 h-24 border-2 border-black border-dashed justify-center items-center rounded-md">
            <Ionicons name="add" size={24} color="black" />
          </TouchableOpacity>
        </ScrollView>

        <Text className="text-x text-black mb-2">Фото референса</Text>
        <ScrollView horizontal className="mb-4">
          {refImages.map((img, i) => (
            <Image key={i} source={{ uri: img.uri }} className="w-24 h-24 mr-2 rounded-md" />
          ))}
          <TouchableOpacity onPress={() => pickImage(setRefImages)} className="w-24 h-24 border-2 border-black border-dashed justify-center items-center rounded-md">
            <Ionicons name="add" size={24} color="black" />
          </TouchableOpacity>
        </ScrollView>
